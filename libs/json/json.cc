...
namespace wampcc {
    ...
    static_assert(std::is_nothrow_move_constructible<json_value>::value,
                  "json_value should be nothrow move constructible");
    ...
    namespace internals {
        ...
        bool valueimpl::is_sint() const {
            ...
            if ((details.type == e_unsigned) &&
                (details.data.uint <= (json_uint_t) (std::numeric_limits<json_int_t>::max)()))
                return true;
...
//----------------------------------------------------------------------
    ...
    json_value json_load_file(const std::string& file_path) {
        ...
        std::string json;
        {
            ...
            if(!infile.is_open())
            {
                throw std::runtime_error
                        { "failed to parse non-existent file: " + file_path };
            }
