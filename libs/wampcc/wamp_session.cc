...
/* Destructor */
    ...
    const char *wamp_session::to_string(wamp_session::state s) {
        switch (s) {
            case wamp_session::state::init :
                return "init";
            case wamp_session::state::recv_hello :
                return "recv_hello";
            case wamp_session::state::sent_challenge :
                return "sent_challenge";
            case wamp_session::state::recv_auth :
                return "recv_auth";
            case wamp_session::state::sent_hello :
                return "sent_hello";
            case wamp_session::state::recv_challenge :
                return "recv_challenge";
            case wamp_session::state::sent_auth :
                return "sent_auth";
            case wamp_session::state::open :
                return "open";
            case wamp_session::state::closing_wait :
                return "closing_wait";
            case wamp_session::state::closing :
                return "closing";
            case wamp_session::state::closed :
                return "closed";
            default:
                return "unknown";
        };
    ...
    void wamp_session::change_state(state expected1, state expected2, state next) {
        ...
        if (is_in(m_state, expected1, expected2)) {
            ...
            LOG_ERROR(m_log_prefix << "state failure, cannot move from " << to_string(m_state) << " to "
                                   << to_string(next));
    ...
    const char *wamp_session::to_string(close_event v) {
        switch (v) {
            case close_event::local_request   :
                return "local_request";
            case close_event::recv_abort      :
                return "recv_abort";
            case close_event::recv_goodbye    :
                return "recv_goodbye";
            case close_event::sock_eof        :
                return "sock_eof";
            case close_event::protocol_closed :
        ...
        try {
            ...
            if (session_mode() == mode::server) {
                ...
                switch (message_type) {
                    ...
                    case msg_type::wamp_msg_heartbeat:
                        return;
...
/* Handles errors for both active & passive sessions */
    ...
    void wamp_session::process_inbound_call(json_array &msg) {
        ...
        auto reply_fn = [wp, request_id](wamp_args args,
            ...
            if (auto sp = wp.lock()) {
                if (!error_uri) {
                    json_array msg{msg_type::wamp_msg_result, request_id, json_object(), args.args_list,
                                   args.args_dict};
                    sp->send_msg(msg);
                } else {
                    json_array msg{msg_type::wamp_msg_error, msg_type::wamp_msg_call, request_id, json_object(),
                                   *error_uri,
                                   args.args_list, args.args_dict};
                    sp->send_msg(msg);
                }
...
/* Used for test purposes */
    ...
    void
    wamp_session::call_error(t_request_id id, std::string uri, json_object details, json_array ja, json_object jo) {
        send_msg({msg_type::wamp_msg_error, msg_type::wamp_msg_call, id, std::move(details), std::move(uri),
                  std::move(ja), std::move(jo)});
    }
