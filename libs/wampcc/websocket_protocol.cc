 ...
 * requests and HTTP responses are case-insensitive. */
                        ...
                        if (m_http_parser->is_upgrade() &&
                            ...
                            if (sec_websocket_protocol_present) {
                                ...
                                int common = m_options.serialisers &
                                             ((has_token(websock_sub, WAMPV2_JSON_SUBPROTOCOL) ? serialiser_type::json
                                                                                               : serialiser_type::none) |
                                              (has_token(websock_sub, WAMPV2_MSGPACK_SUBPROTOCOL)
                                               ? serialiser_type::msgpack : serialiser_type::none));
    ...
    const char *websocket_protocol::to_header(serialiser_type p) {
        switch (p) {
            ...
            case serialiser_type::msgpack:
                return WAMPV2_MSGPACK_SUBPROTOCOL;
        }
        return "";
    }
